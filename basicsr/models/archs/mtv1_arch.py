...
class NAFNet(nn.Module):
    ...
    def forward(self, inp):
        ...
        for encoder, down in zip(self.encoders, self.downs):
            ...
            encs.append(x)
            x = down(x)
        z12 = F.interpolate(encs[0], scale_factor=0.5)
        z21 = F.interpolate(encs[1], scale_factor=2)
        z42 = F.interpolate(encs[2], scale_factor=2)
        z41 = F.interpolate(z42, scale_factor=2)

        encs[1] = self.AFFs[1](z12, encs[1], z42)
        encs[0] = self.AFFs[0](encs[0], z21, z41)
        ...
        # for up, enc_skip, mff in zip(self.ups, encs[::-1], self.MFFs):
        #     x_x = up(x_x)
        #     x_x = x_x + enc_skip
        #     x_x = mff(x_x)
...
class mutv1Local(Local_Base, NAFNet):
