...
class DFFN(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=3, bias=False):
        ...
        self.hconv = nn.Conv2d(hidden_features, hidden_features, kernel_size=1, bias=bias)
        self.wconv = nn.Conv2d(hidden_features, hidden_features, kernel_size=1, bias=bias)
    ...
    def forward(self, x):
        ...
        x_patch_h_conv = self.hconv(x_patch_h)
        x_patch_v_conv = self.hconv(x_patch_v)


        x_patch_h = rearrange(x_patch_h, 'b c h (w patch2) -> b c h w patch2', patch2=self.patch_size)
        x_patch_h_conv = rearrange(x_patch_h_conv, 'b c h (w patch2) -> b c h w patch2', patch2=self.patch_size)
        ...
        x_patch_v_conv = rearrange(x_patch_v_conv, 'b c (h patch1) w -> b c w h patch1', patch1=self.patch_size)
...
class v28Local(Local_Base, NAFNet):
