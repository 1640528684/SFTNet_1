...
class ImageRes1280Model(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageRes1280Model, self).__init__(opt)
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            input_img = F.interpolate(val_data['lq'], size=(704, 1280), mode='bilinear', align_corners=False)
            gt_img = val_data['gt']

            my_dict = {'lq': input_img,
                       'gt': gt_img}
            self.feed_data(my_dict, is_val=True)
            ...
            sr = F.interpolate(visuals['result'], size=(720, 1280), mode='bilinear', align_corners=False)
            sr_img = tensor2img(sr, rgb2bgr=rgb2bgr)
