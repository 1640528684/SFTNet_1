...
class ImageSmlresModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageSmlresModel, self).__init__(opt)
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            gt_img = val_data['gt']
            for H_start in range(5):
                for W_start in range(10):
                    h_end = (H_start + 1) * 128
                    w_end = (W_start + 1) * 128
                    my_dict = {'lq': input_img[:, :, H_start * 128:h_end, W_start * 128:w_end],
                               'gt': gt_img[:, :, H_start * 128:h_end, W_start * 128:w_end]}
            ...
            for W_start in range(10):
                ...
                my_dict = {'lq': input_img[:, :, 592:720, W_start * 128:w_end],
                           'gt': gt_img[:, :, 592:720, W_start * 128:w_end]}
                results.append(my_dict)
            ...
            for H_start in range(5):
                for W_start in range(10):
                    h_end = (H_start + 1) * 128
                    w_end = (W_start + 1) * 128
                    out[:, :, H_start * 128: h_end, W_start * 128: w_end] = deblur_results[i]
                    i = i + 1

            for W_start in range(10):
                w_end = (W_start + 1) * 128
                out[:, :, 592:720, W_start * 128:w_end] = deblur_results[i]
                i = i + 1
