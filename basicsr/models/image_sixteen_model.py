...
class ImageSixteenModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageSixteenModel, self).__init__(opt)
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # val_data['lq'] = F.interpolate(val_data['lq'], size=(768, 1280), mode='bilinear', align_corners=False)
            # val_data['gt'] = F.interpolate(val_data['gt'], size=(768, 1280), mode='bilinear', align_corners=False)
            val_data['lq'] =  val_data['lq'][:,:,:640,:]
            val_data['gt'] =  val_data['gt'][:,:,:640,:]

            # val_data['gt'] = F.pad(val_data['gt'], (0, 0, 0, 48))
