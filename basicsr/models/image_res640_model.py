...
class ImageRes640Model(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageRes640Model, self).__init__(opt)
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            my_dict = {'lq': input_img[:, :, 0:h_end, :],
                       'gt': gt_img[:, :, 0:h_end, :]}
            results.append(my_dict)

            my_dict = {'lq': input_img[:, :, 80:720, :],
                       'gt': gt_img[:, :, 80:720, :]}
            results.append(my_dict)
            ...
            i = 0
            h_end = 640
            out[:, :, 0: h_end, :] = deblur_results[i]
            i = i+1
            out[:, :, 80: 720, :] = deblur_results[i]
