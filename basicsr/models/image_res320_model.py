...
class ImageRes320Model(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageRes320Model, self).__init__(opt)
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            for H_start in range(2):
                for W_start in range(4):
                    h_end = (H_start + 1) * 320
                    w_end = (W_start + 1) * 320
                    my_dict = {'lq': input_img[:, :, H_start * 320:h_end, W_start * 320:w_end],
                               'gt': gt_img[:, :, H_start * 320:h_end, W_start * 320:w_end]}
            ...
            for W_start in range(4):
                ...
                my_dict = {'lq': input_img[:, :, 400:720, W_start * 320:w_end],
                           'gt': gt_img[:, :, 400:720, W_start * 320:w_end]}
                results.append(my_dict)
            ...
            for H_start in range(2):
                for W_start in range(4):
                    h_end = (H_start + 1) * 320
                    w_end = (W_start + 1) * 320
                    out[:, :, H_start * 320: h_end, W_start * 320: w_end] = deblur_results[i]
                    i = i + 1

            for W_start in range(4):
                w_end = (W_start + 1) * 320
                out[:, :, 400:720, W_start * 320:w_end] = deblur_results[i]
                i = i + 1
